...
for _, meta_row in processed.iterrows():
    ...
    team_h1_cum  = get_goals(lfc, '1ST_HALF')
    team_full    = get_goals(lfc, '2ND_HALF')
    opp_h1_cum  = get_goals(opp, '1ST_HALF')
    opp_full    = get_goals(opp, '2ND_HALF')

    team_h2  = (team_full - team_h1_cum) if (not np.isnan(team_h1_cum) and not np.isnan(team_full)) else np.nan
...
print(period_df.groupby('season')[['team_h1', 'team_h2', 'opp_h1', 'opp_h2']].mean().round(2))
...
for s in SEASON_ORDER:
    d = season_period[s].dropna(subset=['team_h1', 'team_h2'])
    if len(d) >= 3:
        _, p = stats.wilcoxon(d['team_h1'], d['team_h2'])
        diff = d['team_h2'].mean() - d['team_h1'].mean()
        print(f'  {SEASON_LABELS[s]}: H1={d["team_h1"].mean():.2f}, H2={d["team_h2"].mean():.2f}, '
              f'diff={diff:+.2f}, p={p:.4f}')

print()
print('Cross-season Team goals — H1:')
k_h1  = season_period['2023-24']['team_h1'].dropna()
y1_h1 = season_period['2024-25']['team_h1'].dropna()
y2_h1 = season_period['2025-26']['team_h1'].dropna()
...
print()
print('Cross-season Team goals — H2:')
k_h2  = season_period['2023-24']['team_h2'].dropna()
y1_h2 = season_period['2024-25']['team_h2'].dropna()
y2_h2 = season_period['2025-26']['team_h2'].dropna()
...
for col_idx, s in enumerate(SEASON_ORDER):
    d = season_period[s].dropna(subset=['team_h1', 'team_h2', 'opp_h1', 'opp_h2'])

    ax_top = axes[0, col_idx]
    bp = ax_top.boxplot(
        [d['team_h1'].values, d['team_h2'].values],
        patch_artist=True,
    ...
    for i, vals in enumerate([d['team_h1'].values, d['team_h2'].values], 1):
        jitter = rng.uniform(-0.12, 0.12, len(vals))
        ax_top.scatter(i + jitter, vals, color=SEASON_COLORS[s], alpha=0.3, s=20, zorder=3)

    means = [d['team_h1'].mean(), d['team_h2'].mean()]
...
for _, meta_row in processed.iterrows():
    ...
    team_row = scores[scores['participant_id'] == 8]
    if len(team_row) == 0:
        skipped_events.append(fid)
        continue
    team_is_home = team_row.iloc[0]['participant'] == 'home'
    ...
    for _, ev in goal_events.iterrows():
        ...
        team_scored = (scorer_is_home == team_is_home)
...
print(goals_df.groupby(['season', 'team_scored']).size().unstack(fill_value=0))
...
for ax, team_scored, panel_title in [
    (axes[0], True,  'Liverpool Goals Scored'),
    (axes[1], False, 'Goals Conceded'),
]:
    subset = goals_df[goals_df['team_scored'] == team_scored]
...
for s in SEASON_ORDER:
    team_scored_s = goals_df[(goals_df['season'] == s) & (goals_df['team_scored'] == True)]
    counts = team_scored_s['bin_15'].value_counts()
...
for s in SEASON_ORDER:
    opp_s = goals_df[(goals_df['season'] == s) & (goals_df['team_scored'] == False)]
...
if len(sub_timing_df) > 0 and len(sub_df) > 0:
    ...
    print(f'\nSubstitution heuristic accuracy (within +/-1 of 2×Team_subs): '
...
ctx_merged = ctx_pivot.merge(period_df[['fixture_id', 'team_h1', 'team_h2', 'opp_h1', 'opp_h2']],
...
print(period_df.groupby('season')[['team_h1', 'team_h2', 'opp_h1', 'opp_h2']]
      .mean().round(2)
      .rename(columns={'team_h1': 'TEAM_H1', 'team_h2': 'TEAM_H2',
                       'opp_h1': 'Opp_H1', 'opp_h2': 'Opp_H2'}))

print()
for s in SEASON_ORDER:
    d = period_df[period_df['season'] == s]
    team_h1 = d['team_h1'].mean()
    team_h2 = d['team_h2'].mean()
    opp_h1 = d['opp_h1'].mean()
    opp_h2 = d['opp_h2'].mean()
    ratio  = team_h2 / team_h1 if team_h1 > 0 else np.nan
    print(f'{SEASON_LABELS[s]}:')
    print(f'  Team goals: H1={team_h1:.2f}, H2={team_h2:.2f} (H2/H1 ratio: {ratio:.2f})')
    print(f'  Goals conceded: H1={opp_h1:.2f}, H2={opp_h2:.2f}')

if len(goals_df) > 0:
    print()
    print('Goal timing bin with most Y2 goals conceded:')
    y2_conc = goals_df[(goals_df['season'] == '2025-26') & (goals_df['team_scored'] == False)]
...
for ax, (side, title) in zip(axes, [(True, 'Liverpool Goals Scored'), (False, 'Goals Conceded')]):
    h1_col = 'team_h1' if side else 'opp_h1'
    h2_col = 'team_h2' if side else 'opp_h2'
